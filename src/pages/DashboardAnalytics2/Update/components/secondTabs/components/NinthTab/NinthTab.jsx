...
export default function NinthTab() {
  ...
  useEffect(() => {
    if (isEmriId) {
      setLoading(true);
      AxiosInstance.get(`FetchIsEmriOlcumDegeri?isemriID=${isEmriId}`)
        .then((response) => {
          console.log("response", response);
          const fetchedData = response.OLCUM_DEGER_LISTE.map((item, index) => ({
            key: item.IDK_PERSONEL_ID,
            sequence: index + 1,
            olcumSeriesNo: item.IDO_SIRANO,
            olcumDescription: item.IDO_TANIM,
            olcumUnit: item.IDO_BIRIM,
            dateTime: dayjs(item.IDO_TARIH).format("DD-MM-YYYY"),
            time: dayjs(item.IDO_SAAT, "HH:mm:ss").format("HH:mm:ss"),
            olcumTargetValue: item.IDO_HEDEF_DEGER,
            olcumMeasurementvalue: item.IDO_OLCUM_DEGER,
            olcumDifference: item.IDO_FARK,
            olcumStatus: item.IDO_DURUM,
            olcumDecimalNumber: item.IDO_FORMAT,
            olcumLimit: item.IDO_MIN_MAX_DEGER,
            olcumMaxValue: item.IDO_MAX_DEGER,
