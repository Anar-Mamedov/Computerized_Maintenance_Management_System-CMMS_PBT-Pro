...
export default function PersonnelDefinitions() {
  ...
  useEffect(() => {
    if (isEmriId) {
      setLoading(true);
      AxiosInstance.get(`IsEmriPersonelList?isemriID=${isEmriId}`)
        .then((response) => {
          console.log("response", response);
          const fetchedData = response.map((item, index) => ({
            key: item.IDK_PERSONEL_ID,
            sequence: index + 1,
            subject: item.IDK_ISIM,
            shift: item.IDK_MASRAF_MERKEZI,
            shiftId: item.IDK_MASRAF_MERKEZI_ID,
            workingtime: item.IDK_SURE,
            hourlyrate: item.IDK_SAAT_UCRETI,
            overtime: item.IDK_FAZLA_MESAI_VAR,
            workinghours: item.IDK_FAZLA_MESAI_SURE,
            overtimepay: item.IDK_FAZLA_MESAI_SAAT_UCRETI,
            cost: item.IDK_MALIYET,
            costCenter: item.IDK_MASRAF_MERKEZI,
