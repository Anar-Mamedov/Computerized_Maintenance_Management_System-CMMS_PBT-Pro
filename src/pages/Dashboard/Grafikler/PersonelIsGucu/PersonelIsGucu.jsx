...
const PersonelIsGucu = () => {
  ...
  const [data, isLoading] = useFetch(
    `GetPersonelBazindaHarcananGuc?startDate=${selectedDate?.personel_is_gucu_zaman[0]}&endDate=${selectedDate?.personel_is_gucu_zaman[1]}`,
    [selectedDate.personel_is_gucu_zaman]
  );
  ...
  if (data) {
    formattedData = data
      .filter((item) =>
        localStorage.getItem("personels")
          ? JSON.parse(localStorage.getItem("personels")).includes(item.ISIM)
          : personels.includes(item.ISIM)
      )
      .map((item) => ({
        ...item,
        Dakika: item.DAKIKA,
  ...
  return (
    <div style={{ width: "100%", height: "100%" }} className="column" id="personel_is_gucu">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h3>
          Personel Bazında Harcanan İş Gücü <br /> ({selectedDate?.personel_is_gucu_zaman[0]} /{" "}
          {selectedDate?.personel_is_gucu_zaman[1]})
        </h3>
