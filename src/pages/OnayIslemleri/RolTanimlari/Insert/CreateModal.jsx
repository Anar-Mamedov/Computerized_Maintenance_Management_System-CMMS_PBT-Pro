...
export default function CreateModal({ workshopSelectedId, onSubmit, onRefresh, secilenIsEmriID }) {
  ...
  const onSubmited = (data) => {
    const Body = {
      ...
      ROL_OLUSTURMA_TAR: dayjs().format("YYYY-MM-DD"),
    };

    AxiosInstance.post(`AddOnayRolTanim`, Body)
      .then((response) => {
  ...
  return (
    <FormProvider {...methods}>
      <div>
        ...
        <Modal width="500px" title="Yeni Rol Ekle" open={isModalVisible} onOk={methods.handleSubmit(onSubmited)} onCancel={handleModalToggle}>
          {loading ? (
