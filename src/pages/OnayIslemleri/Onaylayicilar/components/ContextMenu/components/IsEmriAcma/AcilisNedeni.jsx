...
export default function AcilisNedeni({ disabled }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32830");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32830`)
        .then((response) => {
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("acilisNedeni", value ?? null);
                setValue("acilisNedeniID", value ?? null);
