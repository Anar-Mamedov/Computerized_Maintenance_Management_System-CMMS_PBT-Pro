...
export default function SonucSelect({ disabled, fieldRequirements }) {
  const {
    control,
    setValue,
    formState: { errors },
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32802");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32802`)
        .then((response) => {
