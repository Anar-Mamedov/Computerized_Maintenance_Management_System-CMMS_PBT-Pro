...
export default function MainTable({ isActive, SecilenID }) {
  ...
  const fetch = useCallback(() => {
    setLoading(true);
    AxiosInstance.post(`GetOnayKullaniciList?ONAY_ID=${SecilenID}`)
      .then((response) => {
  ...
  }, [SecilenID]); // secilenIsEmriID değiştiğinde fetch fonksiyonunu güncelle

  useEffect(() => {
    // secilenIsEmriID'nin varlığını ve geçerliliğini kontrol edin
    fetch(); // fetch fonksiyonunu çağırın
  }, [fetch, SecilenID]); // secilenIsEmriID veya fetch fonksiyonu değiştiğinde useEffect'i tetikle
