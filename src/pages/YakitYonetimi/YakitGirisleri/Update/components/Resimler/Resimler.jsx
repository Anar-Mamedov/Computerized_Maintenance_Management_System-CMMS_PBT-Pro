...
const ResimUpload = ({ selectedRowID }) => {
  ...
  const fetchResimIds = async () => {
    try {
      ...
      const [response1] = await Promise.all([AxiosInstance.get(`GetResimIds?RefId=${selectedRowID}&RefGrup=YAKITHRK`)]);
  ...
  const draggerProps = {
    ...
    beforeUpload: (file) => {
      ...
      AxiosInstance.post(`UploadPhoto?refid=${selectedRowID}&refgrup=YAKITHRK`, formData, {
        headers: {
