...
function PersonelBazindaIsGucu(props = {}) {
  ...
  useEffect(() => {
    ...
    watch("baslamaTarihiPersonelIsGucu"),
    watch("bitisTarihiPersonelIsGucu"),
    watch("aySecimiPersonelIsGucu"),
    watch("yilSecimiPersonelIsGucu"),
  ]);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await AxiosInstance.get(
        `GetPersonelBazindaHarcananGuc?startDate=${baslamaTarihi}&endDate=${bitisTarihi}`
      );
  ...
  return (
    ...
    >
      ...
      >
        <Text
          title={`Personel Bazında Harcanan İş Gücü (${
      ...
      >
        ...
        >
          ...
          {data.map((item) => (
            <div key={item.ISIM}>
              <Checkbox
                checked={checkedNames[item.ISIM]}
                onChange={() => handleCheckboxChange(item.ISIM)}
              >
                {item.ISIM}
              </Checkbox>
            </div>
