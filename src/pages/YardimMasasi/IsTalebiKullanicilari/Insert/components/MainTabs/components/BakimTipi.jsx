...
export default function BakimTipi() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32601");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32601`)
        .then((response) => {
  ...
  return (
    ...
    >
      {contextHolder}
      <Text style={{ fontSize: "14px", minWidth: "40px" }}>Kullanıcı Tipi:</Text>
      <Controller
        name="tipi"
        control={control}
