...
function PmBmAdamSaatTrend() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await AxiosInstance.post("GetPmBmTrendChart", body);
  ...
  return (
    <div style={{ width: "100%", height: "100%", padding: "5px" }}>
      <Card
        ...
        title={<Text strong>PM / BM Adam-Saat Trend</Text>}
      >
        {isLoading ? (
          ...
          <div style={{ flex: 1, width: "100%", minHeight: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="ArizaBakim" name="Arıza Bakımı (saat)" fill="#00b5ff" radius={[4, 4, 0, 0]} />
                <Bar dataKey="PlanliBakim" name="Planlı Bakım (saat)" fill="#52c41a" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
...
export default PmBmAdamSaatTrend;