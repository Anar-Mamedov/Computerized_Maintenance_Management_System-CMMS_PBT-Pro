...
function PlanlananTamamlananPeriyot() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await AxiosInstance.post("GetPlanlananTamamlananPeriyotChart", body);
  ...
  return (
    <div style={{ width: "100%", height: "100%", padding: "5px" }}>
      <Card
        ...
        title={<Text strong>Planlanan / Tamamlanan (Periyot)</Text>}
      >
        {isLoading ? (
          ...
          <div style={{ flex: 1, width: "100%", minHeight: 0 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
                ...
                <XAxis dataKey="PeriyotTipi" tick={{ fontSize: 11 }} axisLine={{ stroke: "#e0e0e0" }} tickLine={false} />
                <YAxis tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip cursor={{ fill: "transparent" }} />
                <Legend wrapperStyle={{ fontSize: "11px", paddingTop: "5px" }} />
                
                <Bar dataKey="Planlanan" name="Planlanan" fill="#52c41a" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Tamamlanan" name="Tamamlanan" fill="#00b5ff" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
...
export default PlanlananTamamlananPeriyot;