...
const TarihFilter = () => {
  ...
  // Kanka, default olarak son 3 ayın tarih aralığını state başlangıcına atıyoruz
  ...
  useEffect(() => {
    // Form defaultValues katmanındaki "BaslangicTarihi" ve "BitisTarihi" alanlarına yazıyoruz kanka
  ...
  const handleCancelClick = () => {
    // İptal edilirse veya temizlenirse sistemi yine varsayılan olarak belirlediğin Son 3 Ay durumuna getiriyoruz kanka
    const defaultStart = dayjs().subtract(3, "months");
    const defaultEnd = dayjs();
    
    setStartDate(defaultStart);
    setEndDate(defaultEnd);
    setTimeRange("last3Months");
    setValue("BaslangicTarihi", defaultStart.format("YYYY-MM-DDTHH:mm:ss"));
    setValue("BitisTarihi", defaultEnd.format("YYYY-MM-DDTHH:mm:ss"));
    setOpen(false);
  };
  ...
  return (
      ...
      <div style={{ width: "300px" }}>
        ...
        <div style={{ padding: "10px", display: "flex", flexDirection: "column", gap: "10px" }}>
          <div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
            <DatePicker style={{ width: "100%" }} placeholder="Başlangıç Tarihi" value={startDate} onChange={(date) => { setStartDate(date); setTimeRange(""); }} format="YYYY-MM-DD" />
            <Text style={{ fontSize: "14px" }}>-</Text>
            <DatePicker style={{ width: "100%" }} placeholder="Bitiş Tarihi" value={endDate} onChange={(date) => { setEndDate(date); setTimeRange(""); }} format="YYYY-MM-DD" />
          </div>
          <Select
            style={{ width: "100%" }}
            value={timeRange}
    ...
    } trigger="click" open={open} onOpenChange={setOpen} placement="bottom">
