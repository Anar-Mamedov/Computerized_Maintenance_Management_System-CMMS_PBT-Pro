...
function IsTalebiTipleriAtolyeler(props) {
  const {
    control,
    watch,
    setValue,
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await AxiosInstance.post(`GetMudahaleAnalizAtolyeGraph`, body);
      const formattedData = response.map((item) => ({
        ...item,
        key: item.ISM_ATOLYE_ID,
      }));
      ...
      >
        <div style={{ padding: "10px", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <Text style={{ fontWeight: "500", fontSize: "17px" }}>
            İş Talebi Tipleri / Atölyeler {`(${baslangicTarihi && bitisTarihi ? `${formatDateWithLocale(baslangicTarihi)} / ${formatDateWithLocale(bitisTarihi)}` : ""})`}
          </Text>
        ...
        <div ref={ref1} style={{ display: "flex", flexDirection: "column", gap: "7px", overflow: "hidden", padding: "0px 10px 0 10px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            {/*csv indirme butonu*/}
            <CSVLink data={data} headers={csvHeaders} filename={`İş_Talebi_Tipleri-Atölyeler.csv`} className="ant-btn ant-btn-primary">
...
export default IsTalebiTipleriAtolyeler;
