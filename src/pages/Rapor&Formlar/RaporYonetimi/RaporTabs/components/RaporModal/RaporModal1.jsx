...
function RecordModal({ selectedRow, onDrawerClose, drawerVisible, dataAlreadyLoaded = false }) {
  ...
  const fetchFilters = async () => {
    try {
      ...
      updateReportData({
        ...
        kullaniciRaporu: response[0].KullaniciRaporu,
        selectedRow,
      });
  ...
  return (
    <>
      <Modal destroyOnClose centered title={selectedRow?.RPR_TANIM} open={drawerVisible} onCancel={handleRecordModalClose} footer={null} width="90%" zIndex={1000}>
        <div
          style={{
            marginBottom: "10px",
            display: "flex",
