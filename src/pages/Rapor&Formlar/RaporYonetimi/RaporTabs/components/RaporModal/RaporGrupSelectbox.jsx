...
export default function RaporGrupSelectbox({ form, ...rest }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`RaporGetir?lan=${lan}`);
  ...
  const addItem = () => {
    ...
    if (options.some((option) => option.RPG_ACIKLAMA === trimmedItem)) {
      message.warning("Bu durum zaten var!");
      return;
    }

    setLoading(true);
    AxiosInstance.post(`RaporGrupAdd`, { RPG_ACIKLAMA: trimmedItem })
      .then((response) => {
  ...
  return (
    <>
      ...
      >
        <StyledSelect
          ...
          options={options.map((item) => ({
            value: item.RPR_RAPOR_GRUP_ID,
            label: item.RPG_ACIKLAMA,
          }))}
      ...
      <Form.Item label="Rapor ID" name="reportID" hidden>
        <Input />
      </Form.Item>
    </>
  );
