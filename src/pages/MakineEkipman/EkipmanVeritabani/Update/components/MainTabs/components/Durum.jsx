...
export default function Durum() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32606");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32606`)
        .then((response) => {
  ...
  return (
    ...
    >
      {contextHolder}
      <Text style={{ fontSize: "14px", minWidth: "40px" }}>Durum:</Text>
      <Controller
        name="durum"
        control={control}
