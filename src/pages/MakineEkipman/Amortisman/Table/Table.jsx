...
useEffect(() => {
 ...
 const fetchEquipmentData = async (body, page, size) => {
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnAmortismanOrder");
    const savedVisibility = localStorage.getItem("columnAmortismanVisibility");
    const savedWidths = localStorage.getItem("columnAmortismanWidths");
    ...
    localStorage.setItem("columnAmortismanOrder", JSON.stringify(order));
    localStorage.setItem("columnAmortismanVisibility", JSON.stringify(visibility));
    localStorage.setItem("columnAmortismanWidths", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnAmortismanOrder", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnAmortismanVisibility",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
    ...
    <>
      ...
      {loading ? (
        ...
        cardItems.map((item, index) => (
          ...
          >
            <Text style={{ fontWeight: 600, fontSize: "13px", color: "#5C595C" }}>
              {item.title}
            </Text>
