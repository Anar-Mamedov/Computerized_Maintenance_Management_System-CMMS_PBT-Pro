...
export default function Reason() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("GetIsEmriNedeni");
      if (response && response.is_emri_nedeni) {
        setOptions(response.is_emri_nedeni);
      }
    } catch (error) {
  ...
  return (
    <div
      ...
      {contextHolder}
      <Text style={{ fontSize: "14px" }}>Nedeni:</Text>
      <Controller
        name="Nedeni"
        control={control}
        render={({ field }) => (
