...
const DosyaUpload = ({ selectedRows }) => {
  ...
  const secilenIsEmriID = selectedRows[0].key;
  ...
  const fetchDosyaIds = async () => {
    try {
      setLoading(true);
      const response = await AxiosInstance.get(`GetDosyaList?refId=${secilenIsEmriID}&refGrup=MAKINE`);
  ...
  const handleFileUpload = (file) => {
    ...
    AxiosInstance.post(`UploadFile?refid=${secilenIsEmriID}&refgrup=MAKINE`, formData, {
