...
export default function CikisBoyutu() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32517");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32517`)
        .then((response) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", justifyContent: "space-between", width: "100%" }}>
      {contextHolder}
      <Text style={{ fontSize: "14px" }}>Çıkış Boyutu:</Text>
      <Controller
        name="makineCikisBoyutu"
        control={control}
        render={({ field }) => (
          <Select
            {...field}
            ...
            onChange={(value) => {
              ...
              setValue("makineCikisBoyutuID", value);
