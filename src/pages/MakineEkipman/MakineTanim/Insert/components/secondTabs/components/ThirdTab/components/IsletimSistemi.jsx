...
export default function IsletimSistemi() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32513");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32513`)
        .then((response) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", justifyContent: "space-between", width: "100%" }}>
      {contextHolder}
      <Text style={{ fontSize: "14px" }}>İşletim Sistemi:</Text>
      <Controller
        name="MakineIsletimSistemi"
        control={control}
        render={({ field }) => (
          <Select
            {...field}
            ...
            onChange={(value) => {
              ...
              setValue("MakineIsletimSistemiID", value);
