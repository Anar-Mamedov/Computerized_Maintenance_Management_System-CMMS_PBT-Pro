...
export default function ValfBoyutu() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32515");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32515`)
        .then((response) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", justifyContent: "space-between", width: "100%" }}>
      {contextHolder}
      <Text style={{ fontSize: "14px" }}>Valf Boyutu:</Text>
      <Controller
        name="makineValfBoyutu"
        control={control}
        render={({ field }) => (
          <Select
            {...field}
            ...
            onChange={(value) => {
              ...
              setValue("makineValfBoyutuID", value);
