...
export default function OzelAlan_15() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await AxiosInstance.get("KodList?grup=32534");
    ...
    const addItem = () => {
        if (name.trim() !== "") {
            ...
            setLoading(true);
            AxiosInstance.post(`AddKodList?entity=${name}&grup=32534`)
        .then((response) => {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: "10px", justifyContent: "space-between", width: "100%" }}>
            {contextHolder}
            <Text style={{ fontSize: "14px" }}>Özel Alan 15</Text>
            <Controller
                name="ozelAlan_15"
                control={control}
                render={({ field }) => (
