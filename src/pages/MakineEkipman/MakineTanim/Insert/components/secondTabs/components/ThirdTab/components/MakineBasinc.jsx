...
export default function MakineBasinc() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await AxiosInstance.get("KodList?grup=32519");
    ...
    const addItem = () => {
        if (name.trim() !== "") {
            ...
            setLoading(true);
            AxiosInstance.post(`AddKodList?entity=${name}&grup=32519`)
        .then((response) => {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: "10px", justifyContent: "space-between", width: "100%" }}>
            {contextHolder}
            <Text style={{ fontSize: "14px" }}>Basınç:</Text>
            <Controller
                name="makineBasinc"
                control={control}
                render={({ field }) => (
