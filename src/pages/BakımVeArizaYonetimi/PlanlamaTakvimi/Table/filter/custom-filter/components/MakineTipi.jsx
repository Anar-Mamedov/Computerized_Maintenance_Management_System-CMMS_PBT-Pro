...
export default function MakineTipi() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get("KodList?grup=32501");
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddKodList?entity=${name}&grup=32501`)
        .then((response) => {
  ...
  return (
    <StyledDiv
      ...
      {contextHolder}
      <Text style={{ fontSize: "14px", minWidth: "40px" }}>Makine Tipi:</Text>
      <Controller
        name="makineTipi"
        control={control}
        render={({ field }) => (
