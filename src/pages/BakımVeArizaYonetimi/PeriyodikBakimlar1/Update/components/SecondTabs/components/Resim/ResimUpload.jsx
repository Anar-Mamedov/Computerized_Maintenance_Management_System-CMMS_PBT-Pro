...
const ResimUpload = () => {
  ...
  const fetchResimIds = async () => {
    try {
      ...
      const response = await AxiosInstance.get(`GetResimIds?RefId=${secilenBakimID}&RefGrup=BAKIM`);
  ...
  }, [secilenBakimID, refreshImages]); // refreshImages değişikliklerini de takip eder
  ...
  const draggerProps = {
    ...
    beforeUpload: (file) => {
      ...
      AxiosInstance.post(`UploadPhoto?refid=${secilenBakimID}&refgrup=BAKIM`, formData, {
        headers: {
