...
const normalizeMaterialItem = (item, index) => {
  ...
  return {
    ...
    rowNo: index + 1,
    TB_ISEMRI_MLZ_ID: id,
    StokId: item.StokId ?? item.StokID ?? item.IDM_STOK_ID ?? item.STOK_ID ?? null,
    Kod: item.Kod ?? item.IDM_STOK_KOD ?? item.STK_KOD ?? "",
    MalzemeAdi: item.MalzemeAdi ?? item.IDM_STOK_TANIM ?? item.STK_TANIM ?? "",
    Miktar: quantity,
    BirimId: item.BirimId ?? item.BirimID ?? item.BirimKodId ?? item.IDM_BIRIM_KOD_ID ?? item.STK_BIRIM_KOD_ID ?? null,
    Birim: item.Birim ?? item.IDM_BIRIM ?? item.STK_BIRIM ?? "",
    DepoId: item.DepoId ?? item.DepoID ?? item.IDM_DEPO_ID ?? item.DEPO_ID ?? null,
