...
const ResimUpload = () => {
  ...
  const fetchResimIds = async () => {
    try {
      ...
      const response = await AxiosInstance.get(`GetResimIds?RefId=${secilenBakimID}&RefGrup=ARIZA`);
  ...
  const draggerProps = {
    ...
    beforeUpload: (file) => {
      ...
      AxiosInstance.post(`UploadPhoto?refid=${secilenBakimID}&refgrup=ARIZA`, formData, {
        headers: {
