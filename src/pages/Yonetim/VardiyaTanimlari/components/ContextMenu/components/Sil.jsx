...
export default function Sil({ selectedRows, refreshTableData, disabled, hidePopover }) {
  ...
  const handleDelete = async () => {
    ...
    // selectedRows içindeki doğru ID alanını kullandığından emin ol (genelde 'key' veya 'TB_VARDIYA_ID' olur)
    const idsToDelete = selectedRows.map((row) => row.key || row.TB_VARDIYA_ID);
    ...
    try {
      ...
      const response = await AxiosInstance.post("DeleteVardiya", idsToDelete);

      // Backend'den gelen yanıtı kontrol et
      if (response.status_code === 200 || response.status === 200) {
        message.success("Seçilen vardiyalar başarıyla silindi.");
  ...
  return (
    <Popconfirm
      title="Silme İşlemi"
      description={`${selectedRows.length} adet vardiyayı silmek istediğinize emin misiniz?`}
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: '12px', color: '#8c8c8c', marginTop: '4px', lineHeight: '1.4' }}>
            Seçili vardiyaları kalıcı olarak siler.
          </span>
        </div>
      </div>
    </Popconfirm>
