...
export default function ProsedurNedeni({ disabled = false, fieldRequirements = {}, fieldName = "prosedurNedeni", fieldIdName = "prosedurNedeniID", selectStyle = {} }) {
  const {
    control,
    setValue,
    formState: { errors },
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`GetIsNeden`);
  ...
  const addItem = () => {
    if (newOptionName.trim() !== "") {
      ...
      setLoading(true);
      AxiosInstance.post(`AddIsNeden?entity=${newOptionName}&isTanimId`)
        .then((response) => {
