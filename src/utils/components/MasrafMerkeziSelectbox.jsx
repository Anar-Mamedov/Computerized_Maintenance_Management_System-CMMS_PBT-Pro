...
export default function MasrafMerkeziSelectbox({ disabled = false, fieldRequirements = {}, fieldName = "masrafMerkezi", fieldIdName = "masrafMerkeziID", selectStyle = {} }) {
  const {
    control,
    setValue,
    formState: { errors },
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`MasrafMerkeziList`);
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select
              ...
              options={options
                .filter((item) => item?.TB_MASRAF_MERKEZ_ID !== undefined && item?.TB_MASRAF_MERKEZ_ID !== null)
                .map((item) => {
                  const name = item?.MAM_TANIM?.trim();
                  const code = item?.MAM_KOD?.trim();
                  const label = [code, name].filter(Boolean).join(" - ") || name || code || "";
                  return {
                    value: item.TB_MASRAF_MERKEZ_ID,
                    label,
                  };
                })}
              onChange={(value) => {
